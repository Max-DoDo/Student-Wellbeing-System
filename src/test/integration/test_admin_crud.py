...
# This test file specifically tests CRUD operations and relies on
# creating and tearing down its own database for each test.
# It does NOT inherit from BaseRepositoryTest as that sets up a shared DB.

class TestAdminCRUD(unittest.TestCase):
    def setUp(self):
        # Create a temporary file for the database
        self.temp_db_file = tempfile.NamedTemporaryFile(suffix=".db", delete=False)
        self.temp_db_file.close() # Close the handle so sqlite3 can open it

        self.conn = sqlite3.connect(self.temp_db_file.name)
